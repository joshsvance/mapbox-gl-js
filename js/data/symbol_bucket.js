...
SymbolBucket.prototype.addFeature = function(lines, shapedText, shapedIcon) {
    ...
    var fontScale = layout['text-max-size'] / glyphSize,
        ...
        maxRepeatDistance = Math.max(layout['text-repeat-distance'], layout['icon-repeat-distance']),
    ...
    for (var i = 0; i < lines.length; i++) {
        ...
        var anchors = layout['symbol-placement'] === 'line' ?
            getAnchors(line, symbolMinDistance, maxRepeatDistance, textMaxAngle, shapedText, shapedIcon, glyphSize, textBoxScale, this.overscaling) :
        ...
        for (var j = 0, len = anchors.length; j < len; j++) {
            ...
            // Compare distance between anchors with repeatDistance
            function compareSymbol(textOrIcon, compareTextOrIcon, repeatDistance) {
                if (!(textOrIcon in compareTextOrIcon)) {
                    compareTextOrIcon[textOrIcon] = [];
                    compareTextOrIcon[textOrIcon].push(anchor);    
                } else {
                    for (var k = compareTextOrIcon[textOrIcon].length -1; k >= 0; k--) {
                        if (anchor.dist(compareTextOrIcon[textOrIcon][k]) < repeatDistance ) {
                            anchor.skip = true;
                            break; // If it's within repeatDistance of one anchor, stop looking
                        }
                    }
